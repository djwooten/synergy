...
df = to_synergyfinder(D1, D2, E)
df.to_csv("synergyfinder_comparison/zip_test_data.csv", index=None)

if False:
    # Build ZIP model
    model = ZIP()
    synergy = model.fit(D1, D2, E, use_jacobian=True)

    # Read synergyfinder output
    ...
    ax.scatter(model.synergy, sfdf["synergy"])
    ax.set_xlabel("Python ZIP synergy")
